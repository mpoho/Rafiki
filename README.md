...
## 📺 Écran TFT LCD 3.5" (Shield Parallèle 8-bit)

Un script de test unitaire a été ajouté pour vérifier la dalle et le contrôleur graphique de ton écran :
* **Script de test :** [`rafiki_tft_test/rafiki_tft_test.ino`](file:///C:/Users/Salem/Documents/projet/rafiki/rafiki_tft_test/rafiki_tft_test.ino)
* **Bibliothèques requises :** Installe `Adafruit GFX` et `MCUFRIEND_kbv` via le gestionnaire de bibliothèques de l'IDE Arduino.

> [!CAUTION]
> **Alerte Conflit de Pins (Arduino Uno) :**
> Les écrans shields TFT 3.5" parallèles se branchent directement sur l'Arduino Uno et accaparent presque toutes les broches (Pins A0 à A4 pour le contrôle, et 2 à 9 pour les données).
> Cela entre en conflit direct avec les broches de Rafiki :
> * Les Servomoteurs (Pins 4, 5, 6, 7) et le Buzzer (Pin 8) ne pourront pas fonctionner en même temps que le shield d'écran sur un simple Arduino Uno.
> * **Solutions recommandées pour l'assemblage complet :** 
>   1. Utiliser un **Arduino Mega 2560** (qui offre beaucoup plus de broches et sépare naturellement l'écran des servos).
>   2. Utiliser un écran connecté en liaison **SPI** (qui n'utilise que 4 à 5 broches au lieu de 13).
>   3. Migrer le projet vers un **ESP32** avec extension de ports ou un shield adapté.
